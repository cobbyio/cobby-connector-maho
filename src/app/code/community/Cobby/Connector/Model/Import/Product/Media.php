...
/**
 * Class Cobby_Connector_Model_Import_Product_Media
 */
class Cobby_Connector_Model_Import_Product_Media extends Cobby_Connector_Model_Import_Product_Abstract
{
    ...
    public function import($rows)
    {
        ...
        foreach ($mediaGallery as $prodId => $value) {
            ...
            $result[] = array(
                'product_id' => $prodId,
                Cobby_Connector_Model_Export_Entity_Product::COL_IMAGE_GALLERY => $storedMediaGallery[$prodId],
                Cobby_Connector_Model_Export_Entity_Product::COL_ATTRIBUTES => $attributes,
                'errors' => $value['errors']);
        }
    ...
    private function getAttributes($productIds)
    {
        ...
        foreach ($this->getStoreIds() as $storeId) {
            $attribute = array();
            $collection = Mage::getResourceModel('cobby_connector/catalog_product_collection');
