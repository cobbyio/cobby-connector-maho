...
class Cobby_Connector_Model_Import_Product_Stock extends Cobby_Connector_Model_Import_Product_Abstract
{
    ...
    public function import($rows)
    {
        ...
        $manageStock = Mage::helper('cobby_connector/settings')->getManageStock();
        $defaultQuantity  = Mage::helper('cobby_connector/settings')->getDefaultQuantity();
        $defaultAvailability = Mage::helper('cobby_connector/settings')->getDefaultAvailability();
        ...
        foreach ($rows as $productData) {
            ...
            $stockItem = Mage::getModel('cobby_connector/stock_item');
            ...
            if ($manageStock == Cobby_Connector_Helper_Settings::MANAGE_STOCK_ENABLED) {
                $row = array_merge(
                    $row,
            ...
            elseif ((   $manageStock == Cobby_Connector_Helper_Settings::MANAGE_STOCK_READONLY ||
                        $manageStock == Cobby_Connector_Helper_Settings::MANAGE_STOCK_DISABLED) &&
                        !$existStockData) {
                $stock = array();
            ...
            if ($manageStock == Cobby_Connector_Helper_Settings::MANAGE_STOCK_ENABLED || !$existStockData)
            {
                $result[] = $updateData;
            }
